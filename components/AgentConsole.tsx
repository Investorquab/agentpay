...
export function AgentConsole({
  ...
  const [symbol, setSymbol] = useState("XLM");
  const [city, setCity] = useState("Lagos");
  const [text, setText] = useState("Paste some text to analyze.");

  const inputsFor: Record<string, Record<string, string>> = {
    quote: { symbol },
    weather: { city },
    joke: {},
    wordcount: { text },
  ...
  return (
    <div className="panel p-4 flex flex-col gap-3">
      ...
      <p className="text-sm dim-text leading-relaxed">
        Each button below is a real HTTP 402 call. The agent hits the endpoint, gets a price
        quote back, pays it on Stellar testnet through the connected wallet, and only then
        gets the result.
      </p>
      ...
      <div className="flex flex-col gap-2 mt-1">
        {TOOLS.map((tool) => (
          <div key={tool.id} className="panel-raised px-3 py-2.5 flex items-center gap-3">
            <div className="flex-1 min-w-0">
              ...
              {tool.id === "quote" && (
                <input
                  ...
                  className="mt-2 mono text-xs bg-transparent border-b w-24"
              ...
              {tool.id === "wordcount" && (
                <textarea
                  ...
                  rows={2}
                  className="mt-2 text-xs bg-transparent border rounded px-2 py-1 w-full resize-none"
                  style={{ borderColor: "var(--line)" }}
                />
              )}
            </div>
            ...
            <button
              onClick={() => onRunTool(tool, inputsFor[tool.id])}
      ...
      {!connected && (
        <div className="text-xs dim-text mt-1">Connect a wallet above to enable payments.</div>
      )}
    </div>
  );
}
